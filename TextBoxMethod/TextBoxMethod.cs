...
public static class TextBoxMethod
{
    /// <summary>
    /// 字符串转16进制数组，字符串以空格分割
    /// </summary>
    ...
    /// <param name="nLength"></param>
    /// <returns></returns>
    public static string RemoveSpaceFromString(string strValue)
    {
        string[] strAry = StringToStringArray(strValue, strValue.Length);
    ...
    /// <summary>
    /// 将byte数组的char按照指定长度转存为string
    /// </summary>
    /// <param name="btAryChar"></param>
    /// <param name="nIndex"></param>
    /// <param name="nLen"></param>
    /// <returns></returns>
    public static string ByteCharArrayToString(byte[] btAryChar, int nIndex, int nLen)
    {
        if (nIndex + nLen > btAryChar.Length)
        {
            nLen = btAryChar.Length - nIndex;
        }
        ...
        for (int nloop = nIndex; nloop < nIndex + nLen; nloop++)
        {
            strResult += (char)btAryChar[nloop];
        }
